...
geometry_msgs::Pose bezierInterpolation(const geometry_msgs::Pose& start, const geometry_msgs::Pose& control1, const geometry_msgs::Pose& control2, const geometry_msgs::Pose& end, double t) {
    ...
    // Calculate orientation using Slerp (spherical linear interpolation)
...
class SigmaDevice {
public:
    SigmaDevice(ros::NodeHandle& nh) : nh_(nh), k_0(0.2), alpha(0.05), current_gripper_gap(0.0) { // 添加alpha用于滤波
        id++;
    ...
    int ReadMeasurementsFromDevice() {
        ...
        // 输出当前设备的 position 和 orientation
        ROS_INFO("Sigma7:");
    ...
    void PublishTargetPose()
    {
        ...
        target_pose_msg.orientation = current_pose.orientation; // 直接设置为current_pose的orientation
        ...
        // 更新 previous_target_pose
        previous_target_pose = target_pose_msg;

        // 输出 Pose 和 Orientation
        ROS_INFO("Franka:");
    ...
    void PublishTargetGripperGap() {
        ...
        // 如果 target_gripper_msg 变小时 current_gripper_gap 不变小，则 target_gripper_msg 保持不变
        if (target_gripper_msg.data < current_gripper_gap) {
            target_gripper_msg.data = current_gripper_gap;
        }

        // 输出 Target Gripper Gap
...
public:
    geometry_msgs::WrenchStamped latest_wrench; // force and torque
...
private:
    ...
    geometry_msgs::Pose current_pose; // position and orientation
    std_msgs::Float32 gripper_angle; // [0,PI/6] [0,30]
    double k_0; // 公式中的系数
    double alpha; // 低通滤波器的系数

    ros::Subscriber sub_franka_feedback;
    ros::Subscriber sub_current_end_effector_pose; // 添加的订阅器
    ros::Subscriber sub_gripper_joint_states; // 添加的订阅器

    ros::Publisher pub_target_pose;
    ros::Publisher pub_target_gripper_gap;

    geometry_msgs::Pose current_end_effector_pose; // 存储当前末端执行器位姿
    geometry_msgs::Pose previous_target_pose; // 用于滤波的前一时刻的目标位姿

    double current_gripper_gap; // 当前抓手间隙
...
int holdOnCenter(SigmaDevice* sigma_device) {
    ...
    constexpr double filter_constant = 0.06;
    ...
    while (ros::ok()) {
        ...
        double external_fx = sigma_device->latest_wrench.wrench.force.x / 2;
        double external_fy = sigma_device->latest_wrench.wrench.force.y / 2;
        double external_fz = sigma_device->latest_wrench.wrench.force.z / 2;
        ...
        if (std::abs(external_fx) < 5 && std::abs(external_fy) < 5 && std::abs(external_fz) < 5 &&
