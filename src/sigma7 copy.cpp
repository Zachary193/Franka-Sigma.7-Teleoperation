...
class SigmaDevice {
public:
    SigmaDevice(ros::NodeHandle& nh) : nh_(nh), k_0(0.2), alpha(0.05) { // 添加alpha用于滤波
        id++;
    ...
    void PublishTargetPose()
    {
        ...
        target_pose_msg.position.x = 0.45 + 3 * current_pose.position.x;
        target_pose_msg.position.y = 0 + 3 * current_pose.position.y;
        target_pose_msg.position.z = 0.45 + 3 * current_pose.position.z;
